...
small_train_dataset = tokenized_datasets["train"].shuffle(seed=42).select(range(50000)).map(lambda examples:{'text': examples['text'], 'label': examples['label']},remove_columns=['target', 'SOURCE_FILE', 'tweet_id', 'filename', 'event_type', 'event_type_detail'])
small_eval_dataset = tokenized_datasets["test"].shuffle(seed=42).select(range(10000)).map(lambda examples:{'text': examples['text'], 'label': examples['label']},remove_columns=['target', 'SOURCE_FILE', 'tweet_id', 'filename', 'event_type', 'event_type_detail'])
...
trainer.train("test_trainer/checkpoint-13500")
...
# push to the hub
model.push_to_hub("disaster-mlx-model")#, config=config)
